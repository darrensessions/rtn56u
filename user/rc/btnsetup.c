...
int btn_setup_get_setting(PKT_SET_INFO_GW_QUICK *pkt)	// WLAN 2.4G
{
	...
	strcpy(tmpbuf, nvram_safe_get("rt_ssid"));
	...
	if(nvram_match("rt_auth_mode", "open"))
	{
		...
		pkt->WSetting.Encrypt=atoi(nvram_safe_get("rt_wep_x"));
		if (pkt->WSetting.Encrypt>ENCRYPTION_DISABLE)
		{
			pkt->WSetting.DefaultKey = atoi(nvram_safe_get("rt_key"));
			sprintf(tmpbuf, "rt_key%d", pkt->WSetting.DefaultKey);
...
// 2008.06 James. {
		...
		if(nvram_match("rt_wpa_mode", "2") || nvram_match("rt_crypto", "aes")){
			pkt->WSetting.Auth=AUTHENTICATION_WPA_PSK2;
			pkt->WSetting.Encrypt=ENCRYPTION_AES;
		}
		else if(nvram_match("rt_wpa_mode", "0")|| nvram_match("rt_crypto", "tkip+aes")){
...
// 2008.06 James. }
		strcpy(tmpbuf, nvram_safe_get("rt_wpa_psk"));
...
void btn_setup_save_setting(PKT_SET_INFO_GW_QUICK *pkt)
{
	...
	if (pkt->QuickFlag&QFCAP_WIRELESS)
	{
	   ...
	   if (!(pkt->QuickFlag&QFCAP_GET))
	   {
		...
		nvram_set_str("rt_ssid", pkt->WSetting.SSID, sizeof(pkt->WSetting.SSID));
			...
			(pkt->WSetting.Auth==AUTHENTICATION_WPA_AUTO))
		{
			...
			nvram_set_str("rt_wpa_psk", pkt->WSetting.Key, WPAPSK_LEN);
			nvram_set("rt_key", "1");
			nvram_set("rt_key1", "");
			nvram_set("rt_key2", "");
			nvram_set("rt_key3", "");
...
//	printf("Data Packet %x %x\n", bs_mode, phdr->OpCode);
	...
	switch(phdr->OpCode)
	{
		case NET_CMD_ID_EZPROBE:
		{
		     ...
		     ezprobe_res->isNotDefault = atoi(nvram_safe_get("x_Setting")) | atoi(nvram_safe_get("wsc_config_state")) | atoi(nvram_safe_get("rt_wsc_config_state")); // for EZSetup to coexist w/ WSC
